...
#     participants = {}
#
#     for i, actor in enumerate(bpmn_data['actors']):
#         actor_symbol = actor['symbol']
#         actor_name = actor['actor_name']
#         process_id = f"Process_{actor_symbol}"
#
...
#             gateway_type = gateway['gateway_type']
#
#             # For simplicity, add to relevant process
#             actor_symbol = gateway.get('actor_symbol', list(processes.keys())[0])
#             target_process = processes.get(actor_symbol, list(processes.values())[0])
#
#             # Map gateway types to BPMN elements
#             if 'Parallel' in gateway_type:
#                 ET.SubElement(target_process, 'parallelGateway', {
#                     'id': gateway_symbol,
#                     'name': gateway_type
#                 })
#             elif 'Exclusive' in gateway_type:
#                 ET.SubElement(target_process, 'exclusiveGateway', {
#                     'id': gateway_symbol,
#                     'name': gateway_type
#                 })
#             elif 'Inclusive' in gateway_type:
#                 ET.SubElement(target_process, 'inclusiveGateway', {
#                     'id': gateway_symbol,
#                     'name': gateway_type
#                 })
#             else:
...
#         from_task = flow['from']
#         to_task = flow['to']
#         actor = flow.get('actor', list(processes.keys())[0])
#
#         # Find the process for this actor
#         target_process = processes.get(actor, list(processes.values())[0])
#
#         ET.SubElement(target_process, 'sequenceFlow', {
#             'id': f"Flow_{from_task}_to_{to_task}",
#             'sourceRef': from_task,
...
#
#     # Add message flows
#     for msg_flow in bpmn_data.get('message_flow', []):
#         from_task = msg_flow['from']
#         to_task = msg_flow['to']
#
#         ET.SubElement(collaboration, 'messageFlow', {
...
#     ### NEW ###
#     # === Add BPMNDI Diagram ===
#     bpmn_diagram = ET.SubElement(definitions, f"bpmndi:BPMNDiagram", {
#         'id': 'BPMNDiagram_1'
#     })
#
#     bpmn_plane = ET.SubElement(bpmn_diagram, f"bpmndi:BPMNPlane", {
#         'id': 'BPMNPlane_1',
#         'bpmnElement': 'Collaboration_1'
#     })
#
#     # Position elements on the plane
#     x_pos = 100
#     y_pos = 100
#     lane_height = 300
#     lane_width = 600
...
#     padding = 50
#
#     # Create lanes for participants
#     for i, (symbol, participant) in enumerate(participants.items()):
#         bpmn_lane = ET.SubElement(bpmn_plane, f"bpmndi:BPMNLane", {
#             'id': f'BPMNLane_{symbol}',
#             'bpmnElement': participant.get('id')
#         })
#
#         ET.SubElement(bpmn_lane, f"omgdc:Bounds", {
#             'x': str(x_pos),
#             'y': str(y_pos + i * (lane_height + padding)),
#             'width': str(lane_width),
#             'height': str(lane_height)
#         })
#
#     # Position process elements (tasks, events, gateways)
#     elements_positions = {}
#
#     # Position start events
#     for task in bpmn_data['tasks']:
#         if task['task_symbol'].startswith('S'):
#             actor_symbol = task['actor_symbol']
#             task_symbol = task['task_symbol']
#
#             # Calculate position based on actor's lane
#             lane_y = y_pos + list(participants.keys()).index(actor_symbol) * (lane_height + padding)
#             x = x_pos + padding
#             y = lane_y + padding
#
#             elements_positions[task_symbol] = (x, y)
#
#             bpmn_shape = ET.SubElement(bpmn_plane, f"bpmndi:BPMNShape", {
#                 'id': f'BPMNShape_{task_symbol}',
#                 'bpmnElement': task_symbol
#             })
#
#             ET.SubElement(bpmn_shape, f"omgdc:Bounds", {
#                 'x': str(x),
#                 'y': str(y),
...
#             })
#
#     # Position other tasks and gateways
#     current_x = x_pos + padding * 2
#     for task in bpmn_data['tasks']:
#         if not task['task_symbol'].startswith('S') and not task['task_symbol'].startswith('E'):
#             actor_symbol = task['actor_symbol']
#             task_symbol = task['task_symbol']
#
#             # Calculate position based on actor's lane
...
#     # Position end events
#     for task in bpmn_data['tasks']:
#         if task['task_symbol'].startswith('E'):
#             actor_symbol = task['actor_symbol']
#             task_symbol = task['task_symbol']
#
#             # Calculate position based on actor's lane
#             lane_y = y_pos + list(participants.keys()).index(actor_symbol) * (lane_height + padding)
#             x = x_pos + lane_width - padding - element_width
#             y = lane_y + padding
#
#             elements_positions[task_symbol] = (x, y)
#
...
#     for gateway in gateways:
#         gateway_symbol = gateway['gateway_symbol']
#         actor_symbol = gateway.get('actor_symbol', list(participants.keys())[0])
#
#         # Calculate position based on actor's lane
#         lane_y = y_pos + list(participants.keys()).index(actor_symbol) * (lane_height + padding)
#         x = x_pos + lane_width // 2
#         y = lane_y + padding
#
#         elements_positions[gateway_symbol] = (x, y)
#
#         bpmn_shape = ET.SubElement(bpmn_plane, f"bpmndi:BPMNShape", {
#             'id': f'BPMNShape_{gateway_symbol}',
#             'bpmnElement': gateway_symbol
#         })
#
#         ET.SubElement(bpmn_shape, f"omgdc:Bounds", {
#             'x': str(x),
...
#         })
#
#     # Create BPMNEdges for sequence flows
#     for flow in bpmn_data['control_flow']:
#         from_task = flow['from']
#         to_task = flow['to']
#
#         if from_task in elements_positions and to_task in elements_positions:
#             from_x, from_y = elements_positions[from_task]
#             to_x, to_y = elements_positions[to_task]
#
#             bpmn_edge = ET.SubElement(bpmn_plane, f"bpmndi:BPMNEdge", {
#                 'id': f'BPMNEdge_Flow_{from_task}_to_{to_task}',
#                 'bpmnElement': f'Flow_{from_task}_to_{to_task}'
#             })
#
#             # Create waypoints
#             waypoint1 = ET.SubElement(bpmn_edge, f"omgdi:waypoint", {
#                 'x': str(from_x + element_width // 2),
#                 'y': str(from_y + element_height // 2)
#             })
#
#             waypoint2 = ET.SubElement(bpmn_edge, f"omgdi:waypoint", {
#                 'x': str(to_x + element_width // 2),
#                 'y': str(to_y + element_height // 2)
#             })
#
#     # Create BPMNEdges for message flows
#     for msg_flow in bpmn_data.get('message_flow', []):
#         from_task = msg_flow['from']
#         to_task = msg_flow['to']
#
...
#
#             bpmn_edge = ET.SubElement(bpmn_plane, f"bpmndi:BPMNEdge", {
#                 'id': f'BPMNEdge_MessageFlow_{from_task}_to_{to_task}',
#                 'bpmnElement': f'MessageFlow_{from_task}_to_{to_task}'
#             })
#
#             # Create waypoints
#             waypoint1 = ET.SubElement(bpmn_edge, f"omgdi:waypoint", {
...
#             for task_type_info in bpmn_data.get('task_types', []):
...
#
#             gateway_symbol = gateway.get('gateway_symbol', 'Gateway_' + str(uuid.uuid4())[:8])  ### MODIFIED ###
#             gateway_type = gateway.get('gateway_type', 'Exclusive Gateway')  ### MODIFIED ###
#
#             # Map gateway types to BPMN elements
#             if 'Parallel' in gateway_type:
...
#         # to_task = flow['to', '']
#
#         from_task = flow.get('from', '')  ### MODIFIED ###
#         to_task = flow.get('to', '')  ### MODIFIED ###
#
...
#     bpmn_plane = ET.SubElement(bpmn_diagram, f"bpmndi:BPMNPlane", {
#         'id': 'BPMNPlane_1',
#         'bpmnElement': f'Process_{actor_symbol}'
#     })
#
#     # Position elements on the plane
#     x_pos = 100
...
#     total_elements = len(bpmn_data['tasks']) + len(gateways)
#     total_width = (element_width + padding) * total_elements + padding
#
#     # Position start events
#     for task in bpmn_data['tasks']:
#         if task['task_symbol'].startswith('S'):
...
#             task_symbol = task['task_symbol']
#
#             x = x_pos + total_width - padding - element_width
#             y = y_pos + padding
#
#             elements_positions[task_symbol] = (x, y)
#
...
def generate_collaboration_bpmn(bpmn_data):
    """
    Generate BPMN 2.0 XML for collaboration diagram (multiple actors) with graphical information.
    """
    # BPMN namespaces
    ns = {
        'bpmn': "http://www.omg.org/spec/BPMN/20100524/MODEL",
...
def generate_process_bpmn(bpmn_data):
    """
    Generate BPMN 2.0 XML for process diagram (single actor) with graphical information.
    """
    # BPMN namespaces
    ns = {
        'bpmn': "http://www.omg.org/spec/BPMN/20100524/MODEL",
    ...
    if gateways:
        for gateway in gateways:
            gateway_symbol = gateway.get('gateway_symbol', 'Gateway_' + str(uuid.uuid4())[:8])
            gateway_type = gateway.get('gateway_type', 'Exclusive Gateway')
...
def optimize_bpmn_layout(xml_content, padding=50):
    """彻底解决水平/垂直重叠，强制元素间距"""
    ...
    # 1. 提取所有可见元素（确保不遗漏任何形状）
    elements = {}  # {元素ID: {x, y, width, height, bounds}}
    for shape in root.findall(".//bpmndi:BPMNShape", ns):
        elem_id = shape.get('bpmnElement')  # 如"S1"、"T1"
        if not elem_id:
            continue  # 跳过无ID的元素
        bounds = shape.find("omgdc:Bounds", ns)
        if not bounds:
            continue  # 跳过无边界的元素
        # 解析坐标（强制转换为整数，避免字符串比较问题）
        try:
        ...
        elements[elem_id] = {
            ...
            'bounds': bounds  # 保存边界元素用于修改
        }

    # 2. 按x坐标排序（从左到右处理，确保左侧元素先固定）
    # 排序键：先按x，再按y（确保S1在T1之前处理）
    sorted_ids = sorted(elements.keys(), key=lambda id: (elements[id]['x'], elements[id]['y']))

    # 3. 逐个处理元素，确保与左侧所有元素不重叠
    for i in range(len(sorted_ids)):
        ...
        current_right = current_x + current_width  # 当前元素右边界
        current_bottom = current_y + current_height  # 当前元素下边界

        # 与左侧所有已处理的元素对比
        for j in range(i):
            left_id = sorted_ids[j]
            left = elements[left_id]
            left_x = left['x']
            left_y = left['y']
            left_width = left['width']
            left_height = left['height']
            left_right = left_x + left_width  # 左侧元素右边界
            left_bottom = left_y + left_height  # 左侧元素下边界

            # 检测重叠：水平方向有交集 且 垂直方向有交集
            overlap_horizontal = (current_x < left_right) and (current_right > left_x)
            overlap_vertical = (current_y < left_bottom) and (current_bottom > left_y)

            if overlap_horizontal and overlap_vertical:
                # 强制右移当前元素至左侧元素右边界 + padding
                new_x = left_right + padding
                # 更新当前元素的x坐标和右边界
                current['x'] = new_x
                current['bounds'].set('x', str(new_x))
                current_right = new_x + current_width  # 刷新右边界
    ...
    for edge in root.findall(".//bpmndi:BPMNEdge", ns):
        ...
        if not flow_id:
            continue
        # 找到对应的流程线（sequenceFlow或messageFlow）
        flow = root.find(f".//bpmn:sequenceFlow[@id='{flow_id}']", ns)
        if not flow:
            flow = root.find(f".//bpmn:messageFlow[@id='{flow_id}']", ns)
        if not flow:
            continue
        ...
        # 更新路径点
        waypoints = edge.findall("omgdi:waypoint", ns)
        if len(waypoints) >= 2:
            waypoints[0].set('x', str(source_center_x))
            waypoints[0].set('y', str(source_center_y))
            waypoints[-1].set('x', str(target_center_x))
            waypoints[-1].set('y', str(target_center_y))
...
def generate_bpmn_xml(bpmn_data, output_file=None):
    ...
    # --------------------------
    # 新增：调用 bpmn_layout.js 添加布局
    # --------------------------
    # 定义带布局的输出文件路径（在原文件名后加 "_with_layout"）
    base, ext = os.path.splitext(output_file)
    output_with_layout = f"{base}_with_layout{ext}"

    try:
        print(f"Applying layout via bpmn_layout.js...")
        # 调用 Node.js 脚本：node 脚本路径 输入文件 输出文件
        result = subprocess.run(
            ['node', 'utils/bpmn_layout.js', output_file, output_with_layout],
            check=True,  # 若命令执行失败（返回非0状态码），则抛出异常
            capture_output=True,  # 捕获 stdout 和 stderr
            text=True  # 输出为字符串（而非字节）
        )
        # 打印脚本输出日志（可选，用于调试）
        print(f"Layout applied successfully. Output: {output_with_layout}")
        if result.stdout:
            print(f"Layout tool output: {result.stdout}")
        # 返回带布局的文件路径
        return output_with_layout

    except subprocess.CalledProcessError as e:
        # 若布局工具调用失败，返回原始文件路径
        print(f"⚠️ Failed to apply layout: {e.stderr}")
        print("Using original BPMN file (may lack visual layout)")
        return output_file
    except FileNotFoundError:
        # 若 node 命令未找到（未安装 Node.js）
        print("⚠️ Node.js not found. Please install Node.js to apply layout.")
        print("Using original BPMN file (may lack visual layout)")
        return output_file
...
if __name__ == '__main__':
    try:
        ...
        if bpmn_result:
            ...
            # 假设xml_file是文件路径，先读取内容
            with open(xml_file, 'r', encoding='utf-8-sig') as f:
                xml_content = f.read()

            # 再优化布局
            optimized_xml = optimize_bpmn_layout(xml_content, padding=50)

            with open('optimized_bpmn.xml', 'w', encoding='utf-8') as f:
